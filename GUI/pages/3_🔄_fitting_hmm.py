...
if "indices" not in st.session_state:
    st.error("❌ Indices not found. Please define or load session structure on the **Data Loading** page.")
    st.stop()

# Flag to block further UI
proceed_with_mode = True

# Handle the edge case of only one session
if len(st.session_state.indices) == 1:
    st.warning("⚠️ The current `indices` suggest only one subject or session. This may cause issues with group-level analyses.")
    proceed_with_mode = False  # block mode selection for now

    redefine = st.radio(
        "Do you want to redefine the session structure?",
        ["I want to load indices from file (Page 1)", "Yes, define shape manually", "No, keep current indices",],
        key="redefine_indices_choice"
    )

    if redefine == "Yes, define shape manually":
        ...
        if st.button("Generate indices"):
            ...
            if gamma_length is not None and total_expected != gamma_length:
                st.error(
                    f"❌ The total expected length ({total_expected}) does not match the length of Gamma ({gamma_length})."
                )
            else:
                ...
                proceed_with_mode = True  # allow mode after successful redefinition

    elif redefine == "No, keep current indices":
        proceed_with_mode = True

    elif redefine == "I want to load indices from file (Page 1)":
        st.info("Go to **Page 1: Load Data** and upload the file containing session-wise indices.")
        st.stop()
...
if proceed_with_mode:
    ...
    valid_load_keys = [k for k in st.session_state.keys() if k.startswith("data_") 
                    and not k.startswith("data_behav") 
                    and not k.startswith("data_raw")
                    and not k.startswith("data_FO")
                    and not k.startswith("data_LT")
                    and not k.startswith("data_SR")]
    ####### Configure loaded data for statistical analysis 
    if mode == "Configure loaded data for statistical analysis":
        st.subheader("Configure loaded data for statistical analysis")
        st.markdown("#### Select source of state time courses")

        selected_type = st.radio("Choose data type", ["Gamma", "Viterbi Path"], key="select_data_type")
        Y_key = st.selectbox(f"Select {selected_type}", options=valid_load_keys, key="select_data_key")

        # Only set selected data when button is clicked
        if st.button("Select data", key="select_data_button"):
            ...
            st.session_state.analysis_chosen = False  # reset if reselecting
        ...
        # Proceed only if analysis is confirmed
        if st.session_state.get("run_analysis", False):
            ...
            if not analysis_options:
                st.warning("❌ Could not determine available analysis options. Please ensure Gamma or Viterbi path is loaded.")
            else:
                # Run analysis setup
                analysis_from_gamma(
    ...
    ####### Load and decode existing HMM
        ...
        if hmm_file:
            try:
                ...
                if isinstance(hmm_obj, glhmm.glhmm):
                    ...
                    st.warning("Only the HMM model was loaded. No state time courses or decoded path.")
        ...
        # You can insert your existing HMM decoding logic here

    ####### Train a new HMM
    elif mode == "Train a new HMM":
        st.subheader("Train a new HMM")
        ...
        if detected_type_D in ["Gamma", "Viterbi Path"]:
            st.error(
                ...
                "**Configure loaded data for statistical analysis** (third option above)."
            )
            st.stop()
        if X is not None:
            st.info("You have selected both X and Y variables. The Gaussian Linear HMM (GLHMM) will be used.")

        # Show button and set persistent flag
        if st.button("Select data"):
            st.session_state.show_hmm_config = True

        # Only show config UI if flag is set
        if st.session_state.get("show_hmm_config", False):
            st.markdown("---")
            ...
            if analysis_type == "HMM-MAR":
                ...
                if st.button("Embed Data"):
                    with st.spinner("Embedding data for the HMM-MAR..."):
                ...
                **Number of PCA Components**
                ...
                # Show button and set persistent flag
                if st.button("Embed Data"):
                    with st.spinner("Embedding data for the TDE-HMM..."):
                        # Build the in TDE format
                        X, Y, _ = preproc.build_data_tde(Y,indices,lags=lags,pca=pca)


            if st.button("Confirm and Configure HMM"):
                st.session_state.hmm_confirmed = True

            if st.session_state.get("hmm_confirmed", False):
                ...
                st.markdown("#### Post-Training Options")
                ...
                if st.button("Train HMM"):
                    with st.spinner("Training HMM..."):
                        try:
                            ...
                            st.text_area("Training Log", training_output, height=300)
                            st.session_state.hmm_model = model
                            st.session_state.data_gamma = Gamma
                            ...
                            if st.session_state.save_values:
                                ...
                                if st.session_state.save_vpath and "vpath" in st.session_state:
            ...
            if st.session_state.get("button_config_figures", False):

                # 📊 Configure & Plot HMM Outputs
                st.markdown("---")
                st.subheader("📊 Configure & Plot HMM Outputs")
                analysis_options = []
                if st.session_state.gamma is not None and st.session_state.gamma.ndim == 2:
                    analysis_options.append("Fractional Occupancy")
                    analysis_options.append("Switching Rate")
                    analysis_options.append("Restructure into Trials")
                elif st.session_state.gamma is not None and st.session_state.gamma.ndim == 3 and st.session_state.get("event_markers") is not None:
                    analysis_options.insert(0, "Event-related epoch extraction")
                if st.session_state.vpath is not None and st.session_state.vpath.ndim == 2:
                    analysis_options.append("State Lifetime")
