...
from utils import load_data, create_index_table, same_shape_and_structure, same_shape_except_last, check_indices_match_data_streamlit
...
def data_loading_section():
    ...
    with col1:
        ...
        if st.button("Load Brain Data", key="load_brain"):
            with st.spinner("Loading brain data..."):
                if multi_mode:
                    if folder_path:
                        data_load, st.session_state.indices = load_multiple_files_large(
                            folder_path,
                            filter_string
                        )
                        st.session_state.data_raw = data_load

                        st.session_state.data_load = (
                            np.concatenate(data_load, axis=0) if isinstance(data_load, list) else data_load
                        )

                        st.session_state.indices = (
                            statistics.get_indices_from_list(data_load)
                            if isinstance(data_load, list)
                            else np.array([[0, st.session_state.data_load.shape[0]]])
                        )
    ...
    with col2:
        ...
        file_path_r = None  # make sure it's defined
        ...
        if st.button("Load Behavioural Data", key="load_behav"):
            with st.spinner("Loading behavioural data..."):
                if multi_mode_behav:
                    if folder_path_r:
                        st.session_state.data_behav, _ = load_multiple_files_large(
                            folder_path_r,
                            filter_string_r
                        )
                    else:
                    ...
                    if file_path_r:
                        ...
                        if data_behav is not None:
                            st.session_state.D_and_R_same =same_shape_except_last(st.session_state.data_raw, data_behav)
                            if st.session_state.D_and_R_same:
                                    st.session_state.data_behav = (
                                np.concatenate(data_behav, axis=0) if isinstance(data_behav, list) else data_behav
                            )
                            else:
    ...
    use_idx = st.checkbox("📌 Load indices", value=False)
    if use_idx:
...
def data_summary_section():
    if st.session_state.data_load is None and st.session_state.data_behav is None:
        return
    
    st.header("📊 Data Summary")
    
    if st.session_state.data_load is not None:
        st.subheader("Brain Data")
        ...
        # If 3D, allow reorder and update session_state
        if data_load.ndim == 3:
            ...
            if reorder_button:
                ...
                if st.session_state.D_and_R_same:
                    # Flatten
                    data_flat = statistics.get_concatenate_subjects(data_load)
                    st.session_state.data_load = data_flat
                    data_flat_behav = statistics.get_concatenate_subjects(st.session_state.data_behav)
                    st.session_state.data_behav = data_flat_behav
                    st.success(f"Data has been reshaped and concatenated to 2D: shape {data_flat.shape}")
                else:
                    # Flatten
                    data_flat = statistics.get_concatenate_subjects(data_load)
                    st.session_state.data_load = data_flat
        ...
        # Update variables after potential reshape
        ...
        # Show session indices table if 2D
        if data_final.ndim == 2 and indices is not None:
            ...
            if len(indices)==1:
                st.warning("Only one session loaded. You need to load 'indices data' if you got multiple sessions in your data.")

            
    if st.session_state.data_behav is not None:
        st.subheader("Behavioural Data")
        if st.session_state.D_and_R_same:
            # Update variables after potential reshape
            ...
            if isinstance(st.session_state.data_behav, list):
                st.write(f"**List with a length of:** {len(st.session_state.data_behav)}")
                st.write(f"**Data type: List**")
            else:
                st.write(f"**Shape:** {st.session_state.data_behav.shape}")
                st.write(f"**Data type:** {st.session_state.data_behav.dtype}")
            
            with st.expander("Data Preview"):
                if isinstance(st.session_state.data_behav, (np.ndarray, pd.DataFrame)):
                    st.dataframe(pd.DataFrame(st.session_state.data_behav).head())
                else:
                    st.write(st.session_state.data_behav)
